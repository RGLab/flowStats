...
From the workflow object containing the warped data, we subset a flowSet containing
CD4, CD8 and Time parameters. We try to compare the panels using probability 
binning to identify patients with CD4, CD8 populations different from a control 
flowSet that we create using the data from all the patients.

<<createData>>=
dat <- Data(wf[["Warping"]])
dat<-fsApply(dat,function(x){x[,c("CD4","CD8","Time")]})
@
...
subCount <- nrow(exprs(datComb))/length(dat)
	...
	ctrlData <- ctrlData[,c("CD4","CD8","Time")]
@
...
refBins<-proBin(ctrlData,minRow)
...
statistic(or Probability binning statistic). Patients with sample ids 15, 
3 and 11 appear to be  most different amongst the patient samples.	

<<chiSqstatisticvalues>>=
library(xtable)
