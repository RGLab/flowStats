...
print(densityplot(PatientID~., cytoset_to_flowSet(gs_pop_get_data(gs, "TCells")), channels=pars, 
                  groups=GroupID, scales=list(y=list(draw=F)), 
                  filter=lapply(pars, curv1Filter), layout=c(4,1)))
@ 
...
print(densityplot(PatientID ~ CD69, cytoset_to_flowSet(gs_pop_get_data(gs, "CD4+CD8-")), main = "CD4+",
